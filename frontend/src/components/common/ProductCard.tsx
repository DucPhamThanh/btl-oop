...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleToggleFavorite = (e: React.MouseEvent) => {
    ...
    const isAdmin = user.role?.toUpperCase() === 'ADMIN' || user.role?.toUpperCase() === 'AD' || user.role?.toUpperCase() === 'QT';
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 group overflow-hidden">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-3 right-3 flex flex-col space-y-2">
          ...
          >
            <Heart
              className={`h-4 w-4 ${isFavorite(product.id || product.MaSP || "") ? "fill-current" : ""}`}
            />
          </button>
      ...
      <div className="p-4">
        ...
        <p className="text-sm text-gray-500 mb-1">
          {product.brand?.TenNCC || product.nhaCungCap?.TenNCC || "N/A"}
        </p>
        {/* Name */}
        <h3
          className="font-semibold text-gray-800 mb-2 line-clamp-2 hover:text-blue-600 cursor-pointer"
          onClick={() => onViewDetails(product)}
        >
          {product.name}
        </h3>
